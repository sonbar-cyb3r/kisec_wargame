<?
	...
	$PaymentFailed = $_POST["PaymentFailed"];
	$CostCheck = $_POST["CostCheck"];
	
	if ($PaymentFailed != "N" OR $CostCheck != "Y") {
		echo "<script>alert(\"올바르지 않은 접근 입니다.\");history.back(-1);</script>";
		exit;			
	}
	
	$PrePage = "PaymentCheckAction.php";
	$Referer = $_SERVER['HTTP_REFERER'];
	if (!preg_match("/PaymentCheckAction.php/i", $Referer) AND !preg_match("/PaymentStep2.php/i", $Referer)) {
		echo "<script>alert(\"올바르지 않은 접근 입니다.\");history.back(-1);</script>";
		exit;	
	}
...
<html lang="en">
  <head>
	...
	<div class="modal fade in" id="step1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false" style="display: block;"><div class="modal-backdrop fade in" style="height: 100%;"></div>
	  <div class="modal-dialog">
		<div  class="modal-content">
		  ...
		  <form name="PaymentMethod" action="PaymentAction.php" method="POST">
    ...
    <script src="https://ajax.cloudflare.com/cdn-cgi/scripts/7089c43e/cloudflare-static/rocket-loader.min.js" data-cf-settings="95030fd2899804712514fef1-|49" defer=""></script>
  </body>
</html>
